...
## Initial Challenge: Elevate the Experience of Understanding Abstract Concepts
...
The material-detection layer taught me something the POC didn't: a working pipeline and a *correct* pipeline aren't the same thing. `VNClassifyImageRequest` was returning real, high-confidence labels — just not always the most *useful* one. [Vision](/blog/vision)'s top guess for a real sofa was a generic label like `"structure"`, which doesn't exist in either the soft or hard lookup list, so the naive "just take the top result" approach silently fell back to `.unknown` even when a specific match was sitting a few ranks down.
